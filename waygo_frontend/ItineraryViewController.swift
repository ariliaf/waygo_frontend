...
class ItineraryViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let it = itineraries[indexPath.row]
        
        cell.textLabel?.text = it.name
        if let budget = it.budget, let planDate = it.planDate {
            let budgetStr = "$\(String(budget))"
            let formatter = DateFormatter()
            formatter.timeStyle = DateFormatter.Style.none
            formatter.dateStyle = DateFormatter.Style.short
            let dateStr = formatter.string(from: planDate)
            cell.detailTextLabel.text = "\(dateStr) - $\(budgetStr)"
        }
